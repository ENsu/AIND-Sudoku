...
def grid_values(grid):
    ...
    """
    assert len(grid) == 81
    candidate_grid = ['123456789' if i == '.' else i for i in grid]
    return dict(zip(boxes, candidate_grid))
...
def eliminate(values):
    for box in values:
        if len(values[box]) == 1:
            determined_value = values[box][0]
            for peer in peers[box]:  # remove the determined value from all peers 
                values[peer] = values[peer].replace(determined_value, '')
    return values
...
def only_choice(values):
    for unit in unitlist:
        for i in '123456789':
            digit_position = [box for box in unit if i in values[box]]
            if len(digit_position) == 1:  # only choices
                values[digit_position[0]] = i
    return values
...
def find_min_candidate_node(values):
    for i in range(2, 10):
        for box in values:
            if len(values[box]) == i:
                return box
...
def search(values):
    ...
    if values is False:  # dead end
        return False
    if all(len(values[s]) == 1 for s in boxes):
        return values  # problem solved

    # Choose one of the unfilled squares with the fewest possibilities
    node = find_min_candidate_node(values)
    for candidate in values[node]:
        tmp_values = values.copy()
        tmp_values[node] = candidate
